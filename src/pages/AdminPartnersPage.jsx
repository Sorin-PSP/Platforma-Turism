...
const AdminPartnersPage = () => {
  ...
  const confirmDeletePartner = () => {
    if (partnerToDelete) {
      const result = deletePartner(partnerToDelete.id);
      
      if (result.success) {
        const message = result.deletedOffersCount > 0 
          ? `Partenerul "${result.partner.name}" a fost șters cu succes. S-au eliminat și ${result.deletedOffersCount} oferte asociate.`
          : `Partenerul "${result.partner.name}" a fost șters cu succes.`;
  ...
  return (
    <div>
      ...
      {showDeleteConfirmation && partnerToDelete && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true">
              <div className="absolute inset-0 bg-gray-500 opacity-75"></div>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Sunteți sigur că doriți să ștergeți partenerul <strong>{partnerToDelete.name}</strong>? Această acțiune va elimina definitiv partenerul și toate ofertele asociate. Această acțiune nu poate fi anulată.
                      </p>
                      {partnerToDelete.offersCount > 0 && (
                        <p className="mt-2 text-sm text-red-500">
                          <strong>Atenție:</strong> Acest partener are {partnerToDelete.offersCount} oferte active care vor fi șterse automat din platformă.
                        </p>
                      )}
                    </div>
                  </div>
